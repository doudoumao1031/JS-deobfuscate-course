...
// Helper function to evaluate a binary expression
...
// Helper function to clean the AST
...
// Function to decode a single file
function decodeFile(filePath) {
    ...
    // const outputFilePath = path.join(path.dirname(filePath), 'decoded_' + path.basename(filePath));
    const outputFilePath = path.join(path.dirname('/Users/liang/ProgramFiles/CodeSpace/JS逆向/JS-deobfuscate-course/pg还原/decoded'), 'decoded_' + path.basename(filePath));
    fs.writeFileSync(outputFilePath, decodedCode, { encoding: 'utf-8' });
    console.log(`Decoding complete for ${filePath}. Decoded code written to ${outputFilePath}`);
}

// Get all JavaScript files in the directory and decode them
const directoryPath = '/Users/liang/ProgramFiles/CodeSpace/JS逆向/JS-deobfuscate-course/pg还原/obfuscated';
